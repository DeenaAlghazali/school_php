...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] === "POST") {
        if (isset($_POST['submitSave'])) {
            ...
            if (
                ...
                $sql = "INSERT INTO course (id, name, teacherID) VALUES ('$id', '$name', '$teacher');";
    ...
    ?>
    <h1 class="text">Admin - Home - Add Course</h1>
    <form action="<?php $_SERVER['PHP_SELF'] ?>" method="POST" class="admin-add-teacher">
        <label for="id">Number: </label>
        <input type="text" name="id" id="id" placeholder="Enter Course Number...">
        <br>
        ...
        if (mysqli_num_rows($result) > 0) {
            echo '<select name="teacher" id="teacher" class="custom-select">';
            while ($row = mysqli_fetch_assoc($result)) {
                echo '<option value="' . $row['id'] . '">' . $row['name'] . '</option>';
            }
            echo '</select>';
        } else {
            echo '<h5 style="text-align: center; color: #007bff">no teachers exist</h5>';
        }
        ?>
